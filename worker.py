...
def upload_reports(data, source_id=None):
	"""
	...
	rand_source_id = str(rand_uuid)
	if not source_id:
		source_id = rand_source_id
...
def worker(job_file_path, data, synchronous=True):
	...
	status, source_id, report_ids, is_fhir_resource, report_payload = upload_reports(data, source_id=source_id)
	if not status:
	...
	if 'fhirPatientId' in data:
		patient_id = data['fhirPatientId']
	if 'fhirToken' in data:
		fhir_auth_token = data['fhirToken']
	...
	for de in data_entities:
		...
		de['named_arguments']['fhirVersion'] = fhir_version
